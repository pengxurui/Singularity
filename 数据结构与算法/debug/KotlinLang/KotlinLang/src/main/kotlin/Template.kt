...
object Reader {
    ...
    fun takeFile() {
        IN = BufferedInputStream(File(FILE_PATH).inputStream(), BS)
    }

    val FILE_PATH =
        "/Users/pengxurui/workspace/public/LeetCode-Kotlin/project/KotlinLang/KotlinLang/src/main/kotlin/INPUT"
}
...
fun main(args: Array<String>) {
    ...
    fun dfs() {
        ...
        stack.push(intArrayOf(1, -1, 0))
        while (!stack.isEmpty()) {
            val (root, fa, depth) = stack.pop()
            dp[1] += depth
            son[root] += 1
            for (to in graph[root]) {
                if (to == fa) continue
                stack.push(intArrayOf(to, root, depth + 1))
                son[root] += son[to]
            }
        }
